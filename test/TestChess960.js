...
import {Chess, generateStartPositionFEN, decodeChess960IdFromFEN} from "../src/Chess.js"

const chess960StartPositions = [
...
describe("chess.mjs unit tests for Chess 960", function () {
    ...
    it("should generate a valid FEN for Chess 960 based on the list in chess960.json", () => {
        const chess = new Chess()
        chess960StartPositions.forEach((position, index) => {
            const fen = chess.generate_chess960_position(index)
            console.log(index, position, fen);
            assert.equal(fen, position)
            // assert.equal(decodeChess960IdFromFEN(position), index)
        })
    })
